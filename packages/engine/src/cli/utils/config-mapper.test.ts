...
describe('config-mapper (integration – real zod)', () => {
  ...
  describe('MapperConfigSchema', () => {
    it('applies all default values when parsing an empty object', async () => {
      ...
      expect(result.site.title).toBe('Resource Mapper');
      expect(result.site.subtitle).toBe('Architecture Atlas');
    });
    ...
    it('logo is undefined when not provided', async () => {
      const { MapperConfigSchema } = await import('./config-mapper.js');
      const result = MapperConfigSchema.parse({});
      expect(result.site.logo).toBeUndefined();
    });

    it('throws a ZodError for an invalid build.outDir type', async () => {
      const { MapperConfigSchema } = await import('./config-mapper.js');
      expect(() => MapperConfigSchema.parse({ build: { outDir: 42 } })).toThrow();
    });
  });
  ...
  describe('getMapperConfig', () => {
    ...
    it('throws when mapper.config.json contains invalid JSON', async () => {
      const fs = await getFs();
      (fs.existsSync as ReturnType<typeof vi.fn>).mockReturnValue(true);
    ...
    it('throws a ZodError when config has wrong types', async () => {
      const fs = await getFs();
      (fs.existsSync as ReturnType<typeof vi.fn>).mockReturnValue(true);
      (fs.readFileSync as ReturnType<typeof vi.fn>).mockReturnValue(
        JSON.stringify({ build: { outDir: 123 } })
      );
