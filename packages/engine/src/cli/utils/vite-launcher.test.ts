...
async function setupHappyPath() {
  ...
  mockReq.resolve.mockImplementation((id: string) => {
    if (id === '@k-4u/resource-mapper-core/package.json') return '/mock/core/package.json';
...
describe('ViteLauncher', () => {
  ...
  describe('resolveTemplateRoot (via launch)', () => {
    it('resolves @k-4u/resource-mapper-core package path and passes its dirname as cwd to execa', async () => {
      const { execa } = await getMocks();
    ...
    it('calls logger.error and process.exit(1) when @k-4u/resource-mapper-core cannot be resolved', async () => {
      const { fs } = await getMocks();
      ...
      expect(errorSpy).toHaveBeenCalledWith('Could not find @k-4u/resource-mapper-core. Is it installed?');
