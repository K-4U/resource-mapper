...
export function useGroupGraph() {
    ...
    function buildGraphFromConnections(groupConnections: any[]) {
        const nodes: Node[] = []
        const edges: Edge[] = []
        ...
        const centerY = 400
        const radius = 300
        const angleStep = (2 * Math.PI) / groupConnections.length
        
        groupConnections.forEach((groupConn, index) => {
            ...
            nodes.push({
                id: groupConn.groupName,
                type: 'group',
                position: { x, y },
                data: {
                    label: groupConn.groupName.charAt(0).toUpperCase() + groupConn.groupName.slice(1),
                    description: groupConn.description,
                    serviceCount: groupConn.serviceCount
                }
            })
            
            // Create edges for connections to other groups
            groupConn.connectedToGroups.forEach((targetGroup: string) => {
                edges.push({
                    id: `${groupConn.groupName}-${targetGroup}`,
                    source: groupConn.groupName,
                    target: targetGroup,
                    type: 'smoothstep',
                    animated: true,
                    markerEnd: MarkerType.ArrowClosed
