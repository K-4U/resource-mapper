...
public class ServiceController {
    ...
    @Operation(
            summary = "Get all services for a team",
            description = "Retrieves all services from groups managed by the specified team"
    )
    @ApiResponse(responseCode = "200", description = "Successfully retrieved services for team")
    public ResponseEntity<List<ServiceDefinition>> getServicesByTeam(
            @Parameter(description = "Team ID (lowercase with dashes)", example = "platform-team")
            @PathVariable final String teamId) {
        final List<ServiceDefinition> services = resourceService.getServicesByTeamId(teamId);
        return ResponseEntity.ok(services);
    }

    @GetMapping("/teams/{teamId}/groups")
    @Operation(
            summary = "Get all groups for a team",
            description = "Retrieves all groups managed by the specified team"
    )
    @ApiResponse(responseCode = "200", description = "Successfully retrieved groups for team")
    public ResponseEntity<List<GroupInfo>> getGroupsByTeam(
            @Parameter(description = "Team ID (lowercase with dashes)", example = "platform-team")
            @PathVariable final String teamId) {
        final List<GroupInfo> groups = resourceService.getGroupsByTeamId(teamId);
