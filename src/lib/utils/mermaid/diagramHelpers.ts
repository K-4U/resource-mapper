...
export function buildServiceNodeLines(
  ...
  return {
    nodeId,
    lines: [`${indent}${nodeId}[\"${label}\"]`, `${indent}class ${nodeId} ${nodeClass}`]
  }
}
...
export function getGroupSubgraphId(
  group: Pick<GroupInfo, 'groupName'>,
  kind: GroupSubgraphOptions['kind'] = 'primary'
): string {
...
export function buildGroupSubgraphLines(
  ...
  return [`${indent}subgraph ${groupId}[\"${safeLabel}\"]`, ...bodyLines, `${indent}end`]
}
...
export function withDiagramConfig(diagram: string): string {
  return diagram
}
