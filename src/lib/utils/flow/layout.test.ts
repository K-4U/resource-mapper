...
describe('routeWithLibAvoid', () => {
    ...
    it('generates a direct path when no collision occurs', async () => {
        ...
        const res = await routeWithLibAvoid(nodes[0], nodes[1], {x: 150, y: 20}, Position.Right, {
            x: 200,
            y: 20
        }, Position.Left, nodes);
    ...
    it('routes in a Right-Down-Right shape for offset nodes', async () => {
        ...
        expect(res[0].x).toEqual(150);
        expect(res[0].y).toEqual(20);

        // The first segment should be horizontal (Right) due to the 30px lead-out
        expect(res[1].x).toBeGreaterThanOrEqual(180);
        expect(res[1].y).toBe(20);

        // The last segment should also be horizontal (Right into target Left)
        const lastIdx = res.length - 1;
        expect(res[lastIdx - 1].x).toBeLessThanOrEqual(370);
        expect(res[lastIdx - 1].y).toBe(220);

        expect(res[lastIdx].x).toEqual(400);
        expect(res[lastIdx].y).toEqual(220);
    })
    ...
    it('provides distinct paths for parallel edges to different targets (avoiding overlaps)', async () => {
        ...
        const nodes: Node<FlowNodeData>[] = [
            ...
            serviceNode('b', 'B', undefined, {x: 0, y: 100}),
            serviceNode('c', 'C', undefined, {x: 500, y: 0}),
            serviceNode('d', 'D', undefined, {x: 500, y: 100})
        ];

        // 2. Route two parallel edges
        // Edge AC goes from top-left to top-right
        // Edge BD goes from bottom-left to bottom-right
        const resAC = await routeWithLibAvoid(
            nodes[0], nodes[2],
            {x: 150, y: 20}, 'right',
            {x: 500, y: 20}, 'left',
            nodes, 'edge_0'
        ...
        const resBD = await routeWithLibAvoid(
            nodes[1], nodes[3],
            {x: 150, y: 120}, 'right',
            {x: 500, y: 120}, 'left',
            nodes, 'edge_1'
        ...
        expect(resAC.length, "AC path should have at least 4 segments").toBeGreaterThanOrEqual(4);
        expect(resBD.length, "BD path should have at least 4 segments").toBeGreaterThanOrEqual(4);

        // 4. OVERLAP DETECTION LOGIC
        // We convert the points into normalized segments: "x1,y1-x2,y2"
        // We sort coordinates within the segment string so direction doesn't matter
        const getSegments = (points: { x: number, y: number }[]) => {
            ...
            for (let i = 0; i < points.length - 1; i++) {
                ...
                // Normalize segment string: lower values first to catch overlaps in opposite directions
                const coords = [p1, p2].sort((a, b) => a.x !== b.x ? a.x - b.x : a.y - b.y);
                segments.add(`${coords[0].x},${coords[0].y}_${coords[1].x},${coords[1].y}`);
            }
            return segments;
        };

        const segmentsAC = getSegments(resAC);
        const segmentsBD = getSegments(resBD);

        // 5. Check for Intersections
        const overlaps = [...segmentsAC].filter(seg => segmentsBD.has(seg));

        // Assertion: The set of overlapping segments should be empty
        expect(overlaps, `Found overlapping segments: ${overlaps.join('; ')}`).toHaveLength(0);

        // 6. Check for "Lane" separation
        // If they are orthogonal, they likely have a vertical "trunk"
        const getVerticalX = (points: { x: number, y: number }[]) => {
            const vert = points.find((p, i) => i > 0 && p.x === points[i - 1].x && Math.abs(p.y - points[i - 1].y) > 1);
            return vert ? vert.x : null;
        };

        const xAC = getVerticalX(resAC);
        const xBD = getVerticalX(resBD);

        if (xAC !== null && xBD !== null) {
            expect(Math.abs(xAC - xBD), 'Vertical trunks should be at least 10px apart').toBeGreaterThanOrEqual(10);
        }
    })
    ...
    it('routes around a node in the way', async () => {
        ...
        const hasVertical = res.some((p, i) => i > 0 && Math.abs(p.x - res[i - 1].x) < 0.5 && Math.abs(p.y - res[i - 1].y) > 0.5);
