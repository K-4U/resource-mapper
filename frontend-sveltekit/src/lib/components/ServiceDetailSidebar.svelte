...
<aside class="flex h-full w-96 flex-col border-l border-gray-900/70 bg-gray-950/70">
  <div class="flex-1 overflow-y-auto p-4">
    {#if !group}
      <div class="rounded-xl border border-gray-800/60 bg-gray-900/40 p-4 text-sm text-gray-400">
        Select a group to view details.
      </div>
    {:else}
      <div class="rounded-xl border border-gray-800/60 bg-gray-900/40 p-4">
        <div class="text-xs uppercase tracking-wide text-gray-500">Current Group</div>
        <h2 class="text-xl font-semibold text-white">{group.name}</h2>
        {#if group.description}
          <p class="mt-2 text-sm text-gray-400">{group.description}</p>
        {/if}
        <p class="mt-3 text-xs text-gray-500">ID: {group.groupName}</p>
        {#if group.teamId}
          <div class="mt-4">
      ...
      <div class="mt-4 rounded-xl border border-gray-800/60 bg-gray-900/40 p-4">
        <div class="flex items-center justify-between">
          ...
          {#if service}
            <button
              class="rounded-full border border-gray-600 px-3 py-1 text-xs text-gray-300 hover:bg-gray-800"
        ...
        {#if service}
          <div class="mt-3 text-sm text-gray-400">{service.description ?? 'No description provided.'}</div>
          <div class="mt-4 text-xs text-gray-500">ID: {service.identifier}</div>
